...
# List of side compounds using IDs from the chosen model
...
# Create layout object (coords of each circle, size etc.)
...
```{r}
# This is necessary to get and keep the same layout for all 3 plots: otherwise it regenerates the coordinates each run
...
# Circle div is the range of the circle legend sizes
...
# Fig3
## Barplot
Summary of all predictions across the 3 methods.
```{r}
...
TP_FN_plot =
  ...
  scale_fill_manual(values = pathway_cat_cols_dict[all_methods_df$Type], 
                    labels=c(str_wrap("Insufficient significant metabolites",20),"FN","TP"))+
...
zscores.kod.pathways = pathways_no_comp2 %>% 
  ...
  mutate(value = ifelse(value >= zthr, zthr,  # 6 metabolites capped for plotting purposes
...
This penalises side compounds as even if they are exchangeable, they do not usually add information to the metabolic profile. 
```{r}
length.to.strip = str_length(exch.metab.pattern)+1
...
## Uniqueness
Uniqueness is calculated by thresholding the zscores and looking at how unique each profile is between KOs.
```{r}
thr=2
uniqueness = z.scores %>% pivot_longer(-Metab) %>% 
  ...
  select("Pathway","Name","zscores") %>% 
  group_by(Name) %>% 
  summarise(n=n(), Pathway) %>% 
  group_by(Pathway) %>% 
...
```{r}
# Get a single boxplot if needed
boxplot.df %>% 
...
```{r}
# ggsave("../../plots/exch_ratio_boxplot.png", width = 2.5, height =3, dpi=300, bg="white")
```
...
# Manually create the PCA plot for full customisation
