...
sizes.df = p_vals_all %>% 
  ...
  right_join(pathway_sizes, by = c("subsystem" = "Pathway", "size", "ManualCategoryLvl1")) %>% 
  mutate(profile.size = ifelse(is.na(profile.size), 0, profile.size)) %>% 
...
z.scores %>% pivot_longer(-Metab) %>% 
  ...
  # unique() %>% 
  filter(n!=1) %>% 
  left_join(all_methods_TP_FN_long_df %>% filter(Method == "Human1_GSEA_absolute_metric_external_with_WT_default_all"), by="Pathway") %>% 
  arrange(n)
  # filter(n>10)
  ggplot(aes(x = n))+
  geom_bar()
