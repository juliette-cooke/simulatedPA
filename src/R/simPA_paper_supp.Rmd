...
```{r}
if (model_name=="Human1"){
  lvl1_ss = jsonlite::read_json("../../data/output/lvl1_subsystems_all.json")
  lvl2_ss = jsonlite::read_json("../../data/output/lvl2_subsystems.json")
  biomass_metabs = jsonlite::read_json("../../data/output/biomass_metabs.json")
...
```{r}
biomass_cat_plot
ggsave("../../plots/biomass_plot.png", plot = biomass_cat_plot, dpi=300, width =12, height =6, bg="white")
```

Fig with biomass
```{r}
plot_grid(plot_grid(pathway_sizes_plot, pathway_stats_feas_plot, labels=c("","D"), rel_widths = c(0.35,0.65), nrow=1, vjust=1.7#, vjust=1.9
                    ), NULL,
          biomass_cat_plot, nrow=3, rel_heights = c(0.5,0.03,0.5), labels=c("","","E"), vjust=-0.1, hjust=-2.8#,font.label = list(size = 12)
          )
ggsave(paste0("../../plots/Fig1_biomass_",model_name,"-2.png"), bg="white", dpi=300, width = 11, height = 9.5)
```
...
```
```{r}
# ggsave("../../plots/overlap_heatmap_Human1.png", dpi=300, height =8, width=10, bg="white")
png("../../plots/overlap_heatmap_Human1.png", width = 12.5,height=11, units="in", res=600)
...
zscores.kod.pathways %>%
  ...
  scale_colour_manual(values = pathway_cat_cols_dict[zscores.kod.pathways$Type], name = "GSEA abs prediction")+
  theme_minimal()+
  theme(axis.text.x = element_blank())+
  xlab("KO'd pathways (sorted by std)")+
  ylab("KO'd pathway's metabolite z-scores")+
  guides(col=guide_legend(reverse=TRUE))
```


```{r}
zscore.all.pathways = z.scores.long %>% 
  left_join(all_methods_TP_FN_long_df %>% 
              mutate(Method = str_replace_all(Method, method_names_clean))%>% 
              filter(Method == "GSEA abs"), 
            by = c("Pathway")) %>% 
  arrange(factor(Pathway, zscores.kod.pathways %>% pull(Pathway) %>% unique())) %>% 
  mutate(Pathway = fct_inorder(Pathway)) %>% 
  filter(!is.na(Type))
...
zthr=40
zscores.kod.pathways %>% 
  mutate(p = "Only KO'd pathway metabolites") %>% 
  select(-sd) %>% 
  rbind(zscore.all.pathways %>% 
          select(Pathway, metabID, value, Type) %>% 
          rename("metabolite" = "metabID") %>% 
          mutate(p = "All pathway metabolites")) %>% 
...
```
```{r}
ggsave("../../plots/zscore_boxplots.png", dpi = 300, width=9, height=10, bg="white")
```
...
```{r}
TNR.counts = GSEA.abs.results %>% select(Term, NES, NOM.p.val, unique_id) %>% 
    mutate(TNR = ifelse(NOM.p.val < 0.05, "FP", "TN")) %>% 
  group_by(unique_id, TNR) %>% 
  summarise(counts = n())

"Human1_ORA_pval_005_metrics_external_with_WT_default_all"
...
```{r}
zscores.all.df.plot
ggsave("../../plots/zscores_all_boxplots.png", plot = zscores.all.df.plot, dpi=300, width =10, height =5, bg="white")
...
# Nearby pathways
```{r}
pathway.graph = read.table("/home/juliette/work/code/met4j_code/met4j-1.5.0/simPA/Human1_reaction_dist/Human-GEM-1.11.0_no_blocked_pathway_distance_matrix_weighted.csv", sep=",", header=T) %>% 
...
con.plot.df =
  ...
  filter(n() > 2) %>% 
  left_join(GSEA.abs.results %>% select(Term, NES, NOM.p.val, unique_id), 
...
ggsave("../../plots/all_connected.png", plot=con.plot, dpi=300, bg="white", width = 19, height=15)
```
...
z.scores.long %>% filter(Pathway == pathway1) %>% 
  ...
  # filter(value >)
  group_by(neighbour.pathway) %>% 
  reframe(impact = n(value),n.shared.metabs) %>% 
  # reframe(sd = sd(value), n.shared.metabs) %>% 
  unique() %>% 
  ggplot(aes(x=impact, y=n.shared.metabs))+
  geom_point()#+
...
```
```{r}
ggsave("../../plots/pathway_sizes_TP_FN.png", bg="white", dpi=300, width = 5, height=4)
```
...
times.enriched.TP.FN =  times.enriched %>% 
  ...
  left_join(all_methods_TP_FN_long_df, by = c("Method", "merged.ss" = "Pathway"))
...
```{r}
pca_biplot_cat
ggsave("../../plots/PCA_categories.png", plot = pca_biplot_cat, dpi=300, width =10, height =5, bg="white")
```
...
```
```{r}
ggsave("../../plots/pca_NES_Human1.png", dpi = 300, width=10, height=5, bg="white")
```
...
#                   col.var = "black",
#                   title="", alpha.ind=0)+
#     geom_point(aes(shape=factor(groups$GSEA.abs), fill=factor(groups$GSEA.abs)), size = 3, alpha=0.8)+
#     scale_shape_manual(values = c(23,21), name = "GSEA abs prediction")+
...
# pca_var_plot = 
  # fviz_pca_var(res.PCA, title="", labelsize=3)
# pca_ind_plot
# pca_var_plot
...
```{r}
# # col_pal = c("#FCFFA4","#FCFFA4","#FCFFA4", "#EC6824", "#781B6C", "#000004")
# col_pal = c("#FCFFA4", "#EC6824", "#781B6C", "#000004")
# pca_biplot_NES =
#   fviz_pca_biplot(res.PCA,geom.ind =c("point"), geom.var = c("arrow", "text"), col.var="black", title="", alpha.ind=0,
#                # habillage=factor(groups$GSEA.abs)
#                )+
#     # geom_point(aes(shape=factor(groups$GSEA.abs), fill=factor(groups$GSEA.abs)), size = 3, alpha=0.8)+
#         geom_point(aes(shape=factor(groups$GSEA.abs), fill=(groups$NES)), size = 3, alpha=1)+
#   scale_shape_manual(values = c(23,21), name = "GSEA abs prediction")+
#   # scale_fill_manual(values = pathway_cat_cols_dict[groups$GSEA.abs], name = "GSEA abs prediction")+
#     # scale_fill_continuous(na.value="white")+
#     # scale_fill_gradientn(colours=c("white", "white","darkblue","darkblue"),values = rescale(c(-1,0.6,1.7,2.4)))+
#     # scale_fill_gradientn(colours=col_pal,values = rescale(c(-1,0.6,1.7,2.4)))+
#       scale_fill_gradientn(colours=col_pal,values = scales::rescale(c(0.4,1.7,2.4)), name="NES")+
#   theme(legend.position = "bottom",
#         legend.title =element_text(margin=margin(0,20,0,0)),
#         axis.title.y = element_text(hjust = 0.4))+
...
#   guides(fill = guide_colorbar(theme = theme(legend.title = element_text(vjust = 0.75))))
# 
# # NES_density = groups %>% ggplot(aes(x=NES))+
# #   geom_histogram()+
# #   theme_minimal_grid()+
...
as.data.frame(res.PCA$ind$coord) %>% 
  ...
  left_join(pca.data, by = "subsystem") %>% 
  ggplot(aes(x=Dim.1, y= NES, group=GSEA.abs, col=GSEA.abs)) +
  geom_point()+
...
input.path2 = paste0("../../data/output/",model_name2,"/")
...
network_stats2 = read.table(paste0("../../data/input/",model_name2,"_no_blocked_cytoscape_pathway_graph_statistics.csv"), sep = ",", header = T)
# Data exported from python:
# List of all metabolites and their respective pathway
...
z.scores2 = read.table("/home/juliette/work/pipelines/simpa/Recon2.2/zscores.tsv", sep='\t', header=T)
...
```
```{r}
ggsave(plot=pca_biplot2, "../../plots/Supp_Recon2.2_on_Human1_plot.png", dpi=300, bg="white", width = 9, height=5)
```

# ORA Threshold

NEED TO REEXPORT PVALS
```{r}
p_vals_all = read.table("../../data/output/p_vals_all_df.tsv", sep="\t", header=T) %>% 
...
```
```{r}
ggsave("../../plots/ORA_pval_threshold_scatterplot.png", bg="white", dpi=300, width=10, height=5)
```
